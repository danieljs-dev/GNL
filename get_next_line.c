...
{
	static char		*accumulated_content;
	char			*current_line;
	char			*read_buffer;
	...
	if (!read_buffer)
		return (NULL);
	current_line = get_current_line(fd, read_buffer, accumulated_content);
	free(read_buffer);
	read_buffer = NULL;
	if (current_line == NULL && accumulated_content)
		free(accumulated_content);
	accumulated_content = get_rest_lines(current_line);
